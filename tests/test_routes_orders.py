...
class TestRenameOrderCustomer:
    ...
    def test_moves_every_line_item_to_the_new_customer(self, authed_client, fake_db):
        fake_db.seed(f"{ORDERS}/a", _line(order_id="o1", customer="", item="rice"))
        fake_db.seed(f"{ORDERS}/b", _line(order_id="o1", customer="", item="dal"))
    ...
    def test_modifier_is_stored_alongside_the_name(self, authed_client, fake_db):
        fake_db.seed(f"{ORDERS}/a", _line(order_id="o1", customer=""))
    ...
    def test_other_orders_are_untouched(self, authed_client, fake_db):
        fake_db.seed(f"{ORDERS}/a", _line(order_id="o1", customer=""))
        fake_db.seed(f"{ORDERS}/b", _line(order_id="o2", customer=""))

        authed_client.put("/orders/o1/customer", json={"customer_name": "Ramesh"})

        assert fake_db.docs[f"{ORDERS}/b"]["customer_name"] == ""
    ...
    def test_blank_name_is_rejected(self, authed_client, fake_db):
        ...
        assert fake_db.docs[f"{ORDERS}/a"]["customer_name"] == ""
    ...
    def test_cannot_rename_another_users_order(self, authed_client, fake_db):
        fake_db.seed("users/someone-else/orders/a", _line(order_id="o1", customer=""))

        authed_client.put("/orders/o1/customer", json={"customer_name": "Ramesh"})

        assert fake_db.docs["users/someone-else/orders/a"]["customer_name"] == ""
    ...
    def test_a_legacy_order_can_be_renamed_too(self, authed_client, fake_db):
        ...
        fake_db.seed(f"{ORDERS}/a", _line(order_id=None, customer=""))
        day = _ts(0).date().isoformat()

        resp = authed_client.put(
            f"/orders/legacy|||{day}/customer", json={"customer_name": "Ramesh"}
        )
