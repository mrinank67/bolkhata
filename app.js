...
async function loadSuppliers() {
  ...
  supplierList.innerHTML = '<div class="inventory-empty">Loading suppliers…</div>';
  try {
    const token = await auth.currentUser.getIdToken();
    ...
    supplierList.innerHTML = '<div class="inventory-empty">Could not load suppliers.</div>';
  }
}
...
function renderSupplierList() {
  ...
  if (purchases.length === 0) {
    supplierList.innerHTML = '<div class="inventory-empty">No supplier purchases yet.<br>Add your first purchase below.</div>';
    return;
  }
...
// Add purchase form
$('add-purchase-btn').addEventListener('click', async () => {
  const supplier = $('purchase-supplier').value.trim();
  const item = $('purchase-item').value.trim();
  const qty = parseInt($('purchase-qty').value) || 0;
