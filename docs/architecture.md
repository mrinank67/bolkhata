...
**Orders are the one exception, and they never create stock either.** A `subtract` with a `customer_name` is an order, and an order records what the shopkeeper actually handed over — the catalogue is a pricing and stock-tracking reference, not a list of what may be sold. A catalogued item is fuzzy-matched, priced from `stock.price` and decremented as before; an uncatalogued one is written to `orders/` (and `udhaar/`, when the sale is on credit) under the spoken name, priced only from a spoken rate/amount, and touches no `stock/` doc. Nothing about the miss is persisted: `js/orders.js` recomputes it against `/inventory` on every load — so cataloguing the item later clears the marker by itself — and `routes/bills.py` builds the PDF from the line items alone, which is why the **!** never reaches a customer's bill. A purchase from a supplier not yet in the directory is still recorded — it writes `suppliers_purchases` but no `suppliers/` doc, and the name-canonicalization merge in `routes/suppliers.py` surfaces it on the Suppliers page.
